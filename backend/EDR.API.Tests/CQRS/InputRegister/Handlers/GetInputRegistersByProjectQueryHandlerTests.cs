using Moq;
using EDR.Application.CQRS.InputRegister.Handlers;
...
namespace EDR.API.Tests.CQRS.InputRegister.Handlers
{
    public class GetInputRegistersByProjectQueryHandlerTests
    {
        ...
        {
            // Act & Assert
            Assert.Throws<ArgumentNullException>(() => new GetInputRegistersByProjectQueryHandler(null));
        }

        [Fact]
        public async Task Handle_ExistingProject_ReturnsInputRegistersForProject()
        {
            // Arrange
            var projectId = 1;
            ...
            Assert.All(resultList, item => Assert.Equal(projectId, item.ProjectId));
        ...
        [Fact]
        public async Task Handle_ProjectWithNoInputRegisters_ReturnsEmptyList()
        {
            // Arrange
            var projectId = 999;
