...
namespace NJSAPI.Controllers
{
    ...
    public class WBSOptionsController : ControllerBase
    {
        ...
        public async Task<ActionResult<List<WBSOptionDto>>> GetLevel1Options()
        {
        ...
        [HttpGet("level2")]
        [ProducesResponseType(typeof(List<WBSOptionDto>), 200)]
        public async Task<ActionResult<List<WBSOptionDto>>> GetLevel2Options()
        {
            var query = new GetWBSLevel2OptionsQuery();
            var result = await _mediator.Send(query);
            return Ok(result);
        }
        ...
        {
            var query = new GetWBSLevel3OptionsQuery { Level2Value = level2Value };
            var result = await _mediator.Send(query);
            return Ok(result);
        }
    }
