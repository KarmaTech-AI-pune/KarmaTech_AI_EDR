...
namespace NJS.Application.Services
{
    ...
    public class DeploymentLogService : IDeploymentLogService
    {
        ...
        public void LogDeploymentSuccess(string version, string environment, TimeSpan duration)
        {
            ...
            _logger.LogInformation(
                "✅ DEPLOYMENT SUCCESSFUL - Version: {Version}, Environment: {Environment}, Duration: {Duration:F2}s",
        ...
        public void LogDeploymentFailure(string version, string environment, Exception error, TimeSpan duration)
        {
            ...
            _logger.LogError(error,
                "❌ DEPLOYMENT FAILED - Version: {Version}, Environment: {Environment}, Duration: {Duration:F2}s, Error: {ErrorMessage}",
        ...
        public void LogDatabaseMigration(string version, string migrationName, bool success)
        {
            ...
            {
                _logger.LogInformation(
                    "🗄️ DATABASE MIGRATION SUCCESSFUL - Version: {Version}, Migration: {MigrationName}",
                    version, migrationName);
            }
            else
            {
                _logger.LogError(
                    "🗄️ DATABASE MIGRATION FAILED - Version: {Version}, Migration: {MigrationName}",
                    version, migrationName);
            }
        }
        ...
        public void LogHealthCheck(string version, string status, Dictionary<string, object>? details = null)
        {
            ...
            _logger.Log(logLevel,
                "🏥 HEALTH CHECK - Version: {Version}, Status: {Status}, Details: {Details}",
        ...
        public void LogVersionMismatch(string expectedVersion, string actualVersion, string location)
        {
            ...
            _logger.LogWarning(
                "⚠️ VERSION MISMATCH DETECTED - Expected: {ExpectedVersion}, Actual: {ActualVersion}, Location: {Location}",
