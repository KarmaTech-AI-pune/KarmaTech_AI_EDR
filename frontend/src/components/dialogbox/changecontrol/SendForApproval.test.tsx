...
describe('SendForApproval', () => {
  ...
  beforeEach(() => {
    ...
    mockSendToApprovalBySPM.mockResolvedValue(undefined);
  });
  ...
  it('should call sendToApprovalBySPM and onSubmit on successful submission with comments', async () => {
    render(<SendForApproval {...defaultProps} />);
    await waitFor(() => expect(mockGetProjectById).toHaveBeenCalled());

    const commentsInput = screen.getByLabelText('Comments');
    fireEvent.change(commentsInput, { target: { value: 'Please approve this change.' } });
    ...
    await waitFor(() => {
      expect(mockSendToApprovalBySPM).toHaveBeenCalledWith({
        ...
        comments: 'Please approve this change.',
      });
  ...
  it('should call sendToApprovalBySPM with default comments if none provided', async () => {
    render(<SendForApproval {...defaultProps} />);
    await waitFor(() => expect(mockGetProjectById).toHaveBeenCalled());
  ...
  it('should display error if changeControlApi.sendToApprovalBySPM fails', async () => {
