...
const SendApprovalBox: React.FC<SendApprovalBoxProps> = ({
  ...
  const isReviewLogic = status === WorkflowStatus.SENT_FOR_REVIEW || status === WorkflowStatus.APPROVAL_CHANGES;
  const normalizedStatus = isReviewLogic
  ...
  const dialogTitle = status === WorkflowStatus.APPROVAL_CHANGES
    ? 'Send for Approval'
    : (status === WorkflowStatus.SENT_FOR_REVIEW
      ? 'Decide Review'
      : 'Decide Approval');
