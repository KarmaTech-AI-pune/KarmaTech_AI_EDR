...
vi.mock('@mui/x-date-pickers/DatePicker', () => ({
  DatePicker: ({
    ...
    return (
      <div data-testid={`date-picker-${label.replace(/\s+/g, '-').toLowerCase()}`}>
        <input
          type="date"
          ...
          onBlur={(e: React.FocusEvent<HTMLInputElement>) => { // Add onBlur event
            const newDate = e.target.value ? new Date(e.target.value) : null;
            onChange(newDate);
          }}
          placeholder={label}
        ...
        />
        <button
          data-testid={`date-option-${label.replace(/\s+/g, '-').toLowerCase()}-2025-04-15`}
          onClick={() => onChange(new Date('2025-04-15'))}
        >
          2025-04-15
        </button>
        <button
          data-testid={`date-option-${label.replace(/\s+/g, '-').toLowerCase()}-2025-05-01`}
...
describe('OpportunityForm Component', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(userApi.getUsersByRole).mockImplementation((roleName: string) => {
      ...
      const createAuthUsers = (managers: Array<{ id: string; name: string }>, roleIndex: number) => {
  ...
  describe('Rendering', () => {
    ...
    it('populates form fields with project data when provided', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.getByLabelText(/work name/i)).toHaveValue(mockOpportunity.workName);
        expect(screen.getByTestId('client-input')).toHaveValue(mockOpportunity.client);
        expect(screen.getByLabelText(/client sector/i)).toHaveValue(mockOpportunity.clientSector);
        expect(screen.getByLabelText(/operation/i)).toHaveValue(mockOpportunity.operation);
      });
    });
  });
  ...
  describe('Form Interactions', () => {
    ...
    //   await userEvent.type(screen.getByLabelText(/operation/i), 'New Construction');
  ...
  //     // Click date options
  //     await userEvent.click(screen.getByTestId('date-option-date-of-submission-2025-04-15'));
  //     await userEvent.click(screen.getByTestId('date-option-likely-start-date-2025-05-01'));
  ...
  describe('Data Fetching', () => {
    it('fetches manager data on component mount', async () => {
      ...
      await waitFor(() => {
        expect(userApi.getUsersByRole).toHaveBeenCalledWith('Business Development Manager');
        expect(userApi.getUsersByRole).toHaveBeenCalledWith('Regional Manager');
        expect(userApi.getUsersByRole).toHaveBeenCalledWith('Regional Director');
      });
    });

    it('handles API errors gracefully', async () => {
      // Mock API error
      vi.mocked(userApi.getUsersByRole).mockRejectedValueOnce(new Error('API Error'));
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith('Error fetching managers:', expect.any(Error));
      });
  ...
  describe('Edge Cases', () => {
    ...
    //   // Fill required text fields
    //   await userEvent.type(screen.getByLabelText(/work name/i), 'Minimal Project');
    //   await userEvent.type(screen.getByTestId('client-input'), 'Minimal Client');
    //   await userEvent.type(screen.getByLabelText(/client sector/i), 'Minimal Sector');
    //   await userEvent.type(screen.getByLabelText(/operation/i), 'Minimal Operation');
