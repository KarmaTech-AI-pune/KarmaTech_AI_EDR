...
import { TaskItem } from '../../data/types/dashboard';
...
const TaskPriorityMatrix: React.FC<TaskPriorityMatrixProps> = ({ tasks }) => {
  ...
  const renderTasks = (category: TaskItem['category']) => (
    <Box sx={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 0.5 : 1 }}>
      {tasks
        .filter((task) => task.category === category)
        .map((task) => (
          <Box key={task.id} sx={{ p: isMobile ? 0.75 : 1, backgroundColor: theme.palette.background.paper, borderRadius: 1, fontSize: isMobile ? '0.7rem' : '0.75rem' }}>
            {task.title}
          </Box>
        ))}
    </Box>
