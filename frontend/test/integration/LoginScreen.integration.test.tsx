...
describe('LoginScreen Integration Tests', () => {
  ...
  const mockVersionInfo = {
    ...
    fullVersion: 'v1.3.1-dev.20241225.1',
  ...
  describe('Interactive Version Display Integration', () => {
    ...
    it('makes version clickable and opens release notes modal', async () => {
      ...
      // Click on version
      const versionElement = screen.getByText(/Version 1.3.1/);
      ...
      expect(within(dialog).getByRole('heading', { level: 4 })).toHaveTextContent('v1.3.1');
 
      // Verify release notes API was called
      expect(mockReleaseNotesApi.getReleaseNotes).toHaveBeenCalledWith('1.3.1');
    });
    ...
    it('handles release notes API failure gracefully', async () => {
      ...
      // Should show fallback release notes content for 1.3.1
      await waitFor(() => {
        expect(screen.getByText('Release Notes')).toBeInTheDocument();
      }, { timeout: 10000 });
    });
  ...
  describe('Performance and Loading States', () => {
    ...
    it('handles concurrent API calls correctly', async () => {
      ...
      expect(mockReleaseNotesApi.getReleaseNotes).toHaveBeenCalled();
    });
  });
